        ...
        navController.addOnDestinationChangedListener { _, destination, _ ->
            ...
            when(destination.id) {
                R.id.to_do_list, R.id.done_list -> {
                    ...
                    toolbar.title = getString(R.string.main_page_toolbar_title)
                    navigation.visibility = View.VISIBLE
                    floatingActionButton.show()
                }
                R.id.create_task -> {
                    toolbar.title = getString(R.string.session_create_toolbar_title)
                    navigation.visibility = View.GONE
                    floatingActionButton.hide()
                }
                R.id.tasks_details -> {
                    toolbar.title = getString(R.string.session_page_toolbar_title)
                    navigation.visibility = View.GONE
                    floatingActionButton.hide()
                }
                R.id.edit_task -> {
                    toolbar.title = getString(R.string.edit_session_option)
                }
            }
        }
    }
